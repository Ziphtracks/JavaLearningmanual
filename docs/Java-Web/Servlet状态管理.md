- [ JavaWeb之Servlet状态管理](#head1)
	- [ 一、状态管理介绍](#head2)
		- [1.1 什么是状态管理](#head3)
		- [1.2 为什么需要状态管理](#head4)
		- [1.3 状态管理的两种常见模式](#head5)
	- [ 二、状态管理之Cookie应用](#head6)
		- [2.1 什么是Cookie](#head7)
		- [2.2 创建Cookie对象](#head8)
			- [2.2.1 关于Cookie生命周期的设置](#head9)
			- [2.2.2 关于Cookie共享范围的设置](#head10)
			- [2.2.3 遍历查询Cookie](#head11)
		- [2.3 Cookie的编码与解码问题](#head12)
		- [2.4 发送Cookie的条件](#head13)
		- [2.5 设置Cookie路径](#head14)
		- [2.5 Cookie的优缺点](#head15)
			- [2.5.1 Cookie的优点](#head16)
			- [2.5.2 Cookie的缺点](#head17)
		- [2.6 Cookie综合案例之站点获取上一次访问时间](#head18)
		- [2.7 Cookie综合案例之显示浏览记录](#head19)
	- [ 三、认识域对象](#head20)
		- [3.1 什么是域对象](#head21)
		- [3.2 常见的域对象有什么？](#head22)
	- [ 四、状态管理之Sesstion应用](#head23)
		- [4.1 什么是Session](#head24)
		- [4.2 Session的工作原理](#head25)
		- [4.3 如何获取Session](#head26)
		- [4.4 关于Google怎么查看Cookie](#head27)
		- [4.5 使用Session绑定对象](#head28)
		- [4.6 删除Session对象](#head29)
		- [4.7 Session超时](#head30)
			- [4.7.1 什么是Session超时](#head31)
			- [4.7.2 如何修改Session的缺省时间限制](#head32)
			- [4.7.3 Session失效的几种情况](#head33)
			- [4.7.4 解释浏览器关闭不会让Session失效](#head34)
		- [4.8 关于浏览器禁用Cookie的解决方案【了解】](#head35)
			- [4.8.1 怎么主动禁用浏览器Cookie](#head36)
			- [4.8.2 浏览器禁用Cookie的后果](#head37)
			- [4.8.3 使用Session区分每个用户的方式](#head38)
			- [4.8.3 什么是URL重写](#head39)
			- [4.8.4 如何实现URL重写](#head40)
		- [4.9 Session综合案例值登录（验证码）](#head41)
			- [ c3p0连接池配置文件](#head42)
			- [ 库表操作](#head43)
			- [ login登录页面](#head44)
			- [ User实体类](#head45)
			- [ DBUtils连接池工具类](#head46)
			- [ 验证码工具类](#head47)
			- [ UserDao数据访问层接口](#head48)
			- [ UserDaoImpl数据访问层实现类](#head49)
			- [ 登录校验Servlet](#head50)
			- [ 使用工具来获取验证码](#head51)
			- [ 非工具类获取验证码](#head52)
			- [ 登录展示](#head53)
	- [ 五、Request作为域对象](#head54)
		- [5.1 Request域对象的生命周期](#head55)
		- [5.2 再次理解重定向和转发的区别](#head56)
		- [5.3 Request域对象作用在重定向和转发中的区别](#head57)
	- [ 六、ServletContext对象](#head58)
		- [6.1 什么是ServletContext对象](#head59)
		- [6.2 如何获取ServletContext对象](#head60)
		- [6.3 Servlet上下文的作用及特点](#head61)
			- [6.3.1 Servlet上下文的作用](#head62)
			- [6.3.2 Servlet上下文的特点](#head63)
			- [6.3.3 xml文件配置ServletContext参数](#head64)
		- [6.4 使用ServletContext域来完成站点访问次数案例](#head65)
# <span id="head1"> JavaWeb之Servlet状态管理</span>

------



### <span id="head2"> 一、状态管理介绍</span>



#### <span id="head3">1.1 什么是状态管理</span>
...
#### <span id="head4">1.2 为什么需要状态管理</span>
...
#### <span id="head5">1.3 状态管理的两种常见模式</span>
...
### <span id="head6"> 二、状态管理之Cookie应用</span>



#### <span id="head7">2.1 什么是Cookie</span>
...
#### <span id="head8">2.2 创建Cookie对象</span>
...
##### <span id="head9">2.2.1 关于Cookie生命周期的设置</span>
...
##### <span id="head10">2.2.2 关于Cookie共享范围的设置</span>
...
##### <span id="head11">2.2.3 遍历查询Cookie</span>
...
#### <span id="head12">2.3 Cookie的编码与解码问题</span>
...
#### <span id="head13">2.4 发送Cookie的条件</span>

> 浏览器在发送请求之前，首先会根据请求url中的域名在cookie列表中找所有与当前域名一样的cookie，然后再根据指定的路径进行匹配，如果当前请求在域匹配的基础上还与路径匹配那么就会将所有匹配的cookie发送给服务器。



#### <span id="head14">2.5 设置Cookie路径</span>

> 通过Cookie的setPath方法设置路径



#### <span id="head15">2.5 Cookie的优缺点</span>

##### <span id="head16">2.5.1 Cookie的优点</span>
...
##### <span id="head17">2.5.2 Cookie的缺点</span>
...
#### <span id="head18">2.6 Cookie综合案例之站点获取上一次访问时间</span>
...
#### <span id="head19">2.7 Cookie综合案例之显示浏览记录</span>
...
### <span id="head20"> 三、认识域对象</span>

#### <span id="head21">3.1 什么是域对象</span>

> 可以存储数据的对象，从而实现从对象中存储和取出



#### <span id="head22">3.2 常见的域对象有什么？</span>
...
### <span id="head23"> 四、状态管理之Sesstion应用</span>



#### <span id="head24">4.1 什么是Session</span>
...
#### <span id="head25">4.2 Session的工作原理</span>
...
#### <span id="head26">4.3 如何获取Session</span>
...
#### <span id="head27">4.4 关于Google怎么查看Cookie</span>
...
#### <span id="head28">4.5 使用Session绑定对象</span>
...
#### <span id="head29">4.6 删除Session对象</span>
...
#### <span id="head30">4.7 Session超时</span>

##### <span id="head31">4.7.1 什么是Session超时</span>

> HttpSession的最后一次访问时间和当前时间的差距大于了指定的最大空闲时间，这时服务器就会销毁Session对象。默认的空闲时间为30分钟。（生命周期结束）



##### <span id="head32">4.7.2 如何修改Session的缺省时间限制</span>
...
##### <span id="head33">4.7.3 Session失效的几种情况</span>
...
##### <span id="head34">4.7.4 解释浏览器关闭不会让Session失效</span>
...
#### <span id="head35">4.8 关于浏览器禁用Cookie的解决方案【了解】</span>

##### <span id="head36">4.8.1 怎么主动禁用浏览器Cookie</span>
...
##### <span id="head37">4.8.2 浏览器禁用Cookie的后果</span>
...
##### <span id="head38">4.8.3 使用Session区分每个用户的方式</span>
...
##### <span id="head39">4.8.3 什么是URL重写</span>

> 浏览器在访问服务器上的某个地址时，不再使用原来的那个地址，而是使用经过改写的地址（即：在原来的地址后面加上了sessionID）



##### <span id="head40">4.8.4 如何实现URL重写</span>
...
#### <span id="head41">4.9 Session综合案例值登录（验证码）</span>
...
###### <span id="head42"> c3p0连接池配置文件</span>
...
###### <span id="head46"> DBUtils连接池工具类</span>
...
###### <span id="head48"> UserDao数据访问层接口</span>
...
###### <span id="head49"> UserDaoImpl数据访问层实现类</span>
...
###### <span id="head50"> 登录校验Servlet</span>
...
###### <span id="head51"> 使用工具来获取验证码</span>
...
### <span id="head54"> 五、Request作为域对象</span>



#### <span id="head55">5.1 Request域对象的生命周期</span>

> Request的作用范围是一次访问的过程，生命周期随着访问开始而开始，随着服务器的响应完毕而结束。所以更多的数据会添加到这个域对象



#### <span id="head56">5.2 再次理解重定向和转发的区别</span>
...
#### <span id="head57">5.3 Request域对象作用在重定向和转发中的区别</span>
...
### <span id="head58"> 六、ServletContext对象</span>



#### <span id="head59">6.1 什么是ServletContext对象</span>
...
#### <span id="head60">6.2 如何获取ServletContext对象</span>
...
#### <span id="head61">6.3 Servlet上下文的作用及特点</span>

##### <span id="head62">6.3.1 Servlet上下文的作用</span>
...
##### <span id="head63">6.3.2 Servlet上下文的特点</span>
...
##### <span id="head64">6.3.3 xml文件配置ServletContext参数</span>
...
#### <span id="head65">6.4 使用ServletContext域来完成站点访问次数案例</span>
